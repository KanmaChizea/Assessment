...
const signup: (props: SignupProps) => Promise<RequestResult> = async (
  props: SignupProps,
) => {
  try {
    const user = {
...
const login: (props: LoginProps) => Promise<RequestResult> = async (
  props: LoginProps,
) => {
  try {
...
const addAccount: (props: AddAccountProps) => Promise<RequestResult> = async (
  props: AddAccountProps,
) => {
  try {
    ...
    return {
      isSuccess: false,
      errorMessage: props.newUser
        ? 'Could not create new user'
        : 'Could not add account',
    };
  }
};
const getAccount: (id: string) => Promise<RequestResult> = async (
  id: string,
) => {
  try {
    ...
    const accountList = (data as []).map((e: any) => {
      return {
