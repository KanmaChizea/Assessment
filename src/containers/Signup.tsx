...
export const SignupScreen = () => {
  ...
  const validateFields = () => {
    const newEM = {
      accountNo:
        EmptyStringValidation(accountNo) ??
        NumberValidation({value: accountNo}) ??
        ExactLengthValidation({value: accountNo, length: 10}),
      fullname: EmptyStringValidation(fullname),
      username:
        EmptyStringValidation(username) ??
        NoSpecialCharactersValidation(username),
    ...
    if (!isLoading) {
      ...
      if (isValidate) {
        ...
        if (user.data !== undefined) {
          ...
          dispatch(
            setAccounts({
              accounts: [{accountNo: accountNo, balance: acctbalance}],
            }),
          );
